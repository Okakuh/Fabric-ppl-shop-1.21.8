...
Клиентский мод для Minecraft для поиска табличек с соответствующим текстом.

## Функции
- **Поиск строк**: `/shop <стак> <паттерн>`
- **Поиск по regex**: `/shop_rgx <стак>  <регулярное_выражение>`
- **Визуальная подсветка**: Найденые таблици подсвечиваються сквозь другие блоки
...
- **Функция Quic Shop**: Открывает чат и вводит строку "/shop 64 " по нажатию комбинации клавиш
...
### Логический поиск
```bash
/shop 1 алмаз+ ру да               # (И "алмаз" И " ру да") На таблице должны быть все части поиска разделенные знаком "+"
/shop 1 алмаз -уголь               # (ИЛИ "алмаз " ИЛИ "уголь")  На таблице должна быть хотя бы одна часть поиска разделенная знаком "-"
